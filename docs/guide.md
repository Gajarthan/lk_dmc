...
An example variable list is in [`.env.example`](../.env.example); the CLI does not automatically
...
The README workflow runs after collection workflow completion, every two hours,
and on manual request. It reads summaries from the current repository's data branches
through the GitHub Contents API, using the built-in token for private repositories.
It only updates the marked archive section in the [dashboard](../README.md), preserving the surrounding content.
...
Dashboard totals count archived metadata, PDFs, extracted text, validated analysis,
and reports with tables. Analysis counts exclude results whose source text changed.
Each dataset summary includes its eight newest reports; the dashboard shows up to
eight across all datasets. Missing metrics are shown as **Not reported**. The
timestamp reflects the newest published summary, rather than the page viewing time.
